...
Whether to enable CJK-friendly emphasis and strikethrough parsing. When enabled, `**`, `*`, and `~~` will correctly parse as emphasis/strikethrough even when adjacent to CJK punctuation characters (e.g. fullwidth parentheses, periods, commas).

This addresses a [CommonMark spec limitation](https://github.com/commonmark/commonmark-spec/issues/650) where emphasis markers fail next to CJK punctuation. For example, `**中文文本（带括号）。**` would not render as bold without this option.
