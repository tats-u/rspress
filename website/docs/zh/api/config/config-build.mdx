...
是否启用对 CJK（中日韩）友好的强调和删除线解析。启用后，`**`、`*` 和 `~~` 在紧邻 CJK 标点符号（如全角括号、句号、逗号）时也能正确解析为强调或删除线。

此选项解决了 [CommonMark 规范的一个限制](https://github.com/commonmark/commonmark-spec/issues/650)，该限制导致强调标记在 CJK 标点旁无法正常工作。例如，未启用此选项时 `**中文文本（带括号）。**` 不会被渲染为粗体。
